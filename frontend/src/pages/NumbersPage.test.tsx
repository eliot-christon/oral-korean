 ...
 *
 * Write-mode note: `NumbersPage.tsx` and `api.ts` now implement the real page (T05 is
 * done); every test below runs against that implementation, not a stub. `fetch` is stubbed
 * per-test with canned JSON responses shaped exactly like T04's real routes
 * (`tests/test_api_numbers.py` pins the Python side of the same contract);
 * `HTMLMediaElement.prototype.play` is stubbed globally since jsdom does not implement it.
 * No network, no real audio, ever.
 *
 ...
 *
 * The page lived in `App.tsx` until vocab-words-ui T01 moved it, and this file with it, to
 * `pages/`, when `App.tsx` became the app shell. The move changed only imports and the
 * rendered component: every test below kept its name, stubs and assertions.
 *
 ...
 * `render(<NumbersPage />)` calls across this file's many tests would stack up in
 * `document.body`.
 ...
 * constraints on `NumbersPage.tsx` the implementation must satisfy for these tests to pass):
 ...
 * Any of these is a reasonable default, not the only valid one; if the real `NumbersPage.tsx`
...
/** Stubs `fetch` with canned responses for the three endpoints `NumbersPage` is expected to call. */
...
beforeEach(() => {
  ...
  // <audio> element that the page is expected to call .play() on.
...
test('draws a question on load and sets the audio elements source from the response', async () => {
  ...
  const { container } = render(<NumbersPage />)
...
test('shows the backends own rejection message when creating a question is refused', async () => {
  ...
  // `NumbersPage` must surface the real detail text (e.g. the actual range a system supports),
